...
try:
    for file in sorted((OUT/'exports').glob('*.fbx')):
        ...
        task=unreal.AssetImportTask();task.filename=str(file);task.destination_path='/Game/QuietArray'
        ...
        for path in task.imported_object_paths:
            ...
            assert 1000 < max(size) < 5000, ('Expected metre-authored geometry in centimetres',size)
    ...
    editor.new_level('/Game/QuietArray/Review')
...
unreal.log('QUIET_ARRAY_IMPORT '+json.dumps(report))
