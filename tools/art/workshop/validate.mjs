...
const MODEL = 'nomad-workshop';
const ROOTS = ['NomadDriveCore', 'NomadLegServiceHatch'];
const LIMITS = { NomadDriveCore: [1.4, 1.8, 1.3], NomadLegServiceHatch: [0.38, 0.07, 0.43] };
const EPS = 0.002;
...
function inspectGeometry(gltf) {
  ...
  if (nodes.some((node, index) => node.mesh !== undefined && !visitedMeshes.has(index)))
    throw new Error('Export contains geometry outside the two workshop roots');
  return result;
}
...
const expected = JSON.parse(await readFile('assets/workshop/bounds.json', 'utf8'));
...
for (const name of ROOTS) {
  ...
  if (
    [0, 2].some(
      (axis) =>
        actual.min[axis] < -LIMITS[name][axis] - EPS || actual.max[axis] > LIMITS[name][axis] + EPS,
    ) ||
    actual.min[1] < -EPS ||
    actual.max[1] > LIMITS[name][1] + EPS
  )
    errors.push('workshop model exceeds unchanged collider or flush panel bounds');
  if (actual.triangles > 24000) errors.push('root exceeds 24k triangle budget');
  geometry[name] = { ...actual, reference, errors };
}
const totalTriangles = Object.values(geometry).reduce((sum, item) => sum + item.triangles, 0);
if (totalTriangles > 30000)
  throw new Error(`whole kit exceeds 30k triangle budget (${totalTriangles})`);
...
await writeFile('assets/workshop/gltf-validation.json', JSON.stringify(report, null, 2));
console.log(
  JSON.stringify(
