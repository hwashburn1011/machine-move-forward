...
report=[]
for file in (OUT/'exports').glob('*.glb'):
...
manifest=json.loads((OUT/'source/asset_manifest.json').read_text())
for item in manifest['exports']:item['bytes']=(OUT/'exports'/item['file']).stat().st_size
(OUT/'source/asset_manifest.json').write_text(json.dumps(manifest,indent=2));(OUT/'source/tangent_cleanup.json').write_text(json.dumps(report,indent=2));print(report)
