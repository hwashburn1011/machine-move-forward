...
ROOT=Path(__file__).resolve().parents[3];OUT=ROOT/'assets/fieldwork'
tools=unreal.AssetToolsHelpers.get_asset_tools()
report={'engine':unreal.SystemLibrary.get_engine_version(),'assets':[]}
try:
    for file in [OUT/'exports'/(name+'.fbx') for name in ['L12','CaretakerDock','RifleStabilizer','RifleBurstCam','ShotgunChoke','ShotgunScatterBrake']]:
        ...
        task=unreal.AssetImportTask();task.filename=str(file);task.destination_path='/Game/NomadFieldwork'
        ...
        for path in task.imported_object_paths:
            ...
            assert 5 < max(size) < 300, ('Expected metre-authored geometry in centimetres',size)
            report['assets'].append({'name':file.stem,'path':path,'size_cm':size,'materials':len(asset.get_editor_property('static_materials'))})
    assert len(report['assets'])==6, report
    editor=unreal.get_editor_subsystem(unreal.LevelEditorSubsystem)
    editor.new_level('/Game/NomadFieldwork/Review')
...
unreal.log('FIELDWORK_IMPORT '+json.dumps(report))
