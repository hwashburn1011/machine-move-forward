...
const INPUT = process.env.MMF_GLTF_INPUT || path.join(ROOT, 'assets', 'iron-nomad', 'exports');
const OUTPUT = process.env.MMF_GLTF_OUTPUT || path.join(ROOT, 'assets', 'iron-nomad', 'optimized');
...
async function optimizeOne(file) {
  ...
  for (let imageIndex = 0; imageIndex < originalImages.length; imageIndex++) {
    ...
    const limit = path.basename(file).includes('-full') ? (smallDetail ? 1024 : 2048) : (smallDetail ? 512 : 1024);
    const prepared = sharp(png).resize({ width: limit, height: limit, fit: 'inside', withoutEnlargement: true });
    ...
    imageReport.push({ image: image.name || `image_${imageIndex}`, roles: role, width: Math.min(limit, meta.width), height: Math.min(limit, meta.height), sourceWidth: meta.width, sourceHeight: meta.height, pngBytes: png.length, webpBytes: webp.length, format: 'webp', lossless: !isBase });
  }
  ...
  const rootNode = (json.nodes || []).find((node) => node.extras?.assetId === 'iron-nomad');
  const rootExtras = rootNode?.extras || {};
  const stem = path.basename(file, '.glb');
  const required = ['IronNomad_FourLegWalker', 'CargoCrane_Yaw', 'Turbine_Rotor', 'PlayerSpawn', 'Exhaust_A', 'Exhaust_B', ...['FrontLeft','FrontRight','RearLeft','RearRight'].map(id=>'Leg_'+id+'_Hip')];
...
await fs.writeFile(process.env.MMF_GLTF_REPORT || path.join(OUTPUT, 'optimize-manifest.json'), JSON.stringify({ version: 1, generatedAt: new Date().toISOString(), encoder: 'meshoptimizer 1.1.1', texturePolicy: 'Full: baseColor/emissive up to 2K, normal/ORM up to 1K. Game: baseColor/emissive up to 1K, normal/ORM up to 512. WebP base q92, data maps lossless. Original PNG masters retained. GPU residency is uncompressed.', geometryPolicy: 'EXT_meshopt_compression; articulated nodes and animation retained.', assets: reports }, null, 2) + '\n');
