/**
 * Optimize complete expansion-v1 GLBs without changing their authored graph.
 *
...
const INPUT = path.join(ROOT, 'assets', 'expansion-v1', 'staging');
const OUTPUT = path.join(ROOT, 'assets', 'expansion-v1', 'optimized');
...
async function optimizeOne(file) {
  ...
  for (let imageIndex = 0; imageIndex < originalImages.length; imageIndex++) {
    ...
    const prepared = smallDetail ? sharp(png).resize({ width: 512, height: 512, fit: 'fill' }) : sharp(png);
    ...
    imageReport.push({ image: image.name || `image_${imageIndex}`, roles: role, width: smallDetail ? 512 : meta.width, height: smallDetail ? 512 : meta.height, sourceWidth: meta.width, sourceHeight: meta.height, pngBytes: png.length, webpBytes: webp.length, format: 'webp', lossless: !isBase });
  }
...
await fs.writeFile(path.join(OUTPUT, 'optimize-manifest.json'), JSON.stringify({ version: 1, generatedAt: new Date().toISOString(), encoder: 'meshoptimizer 1.1.1', texturePolicy: 'optimized GLBs require EXT_texture_webp and select baseColor/emissive WebP q92 at 1K or normal/ORM lossless WebP at 512; original 1K PNG masters remain in staging; WebP does not reduce GPU residency', geometryPolicy: 'EXT_meshopt parents use a placeholder fallback buffer sized for decoded accessors; compressed payloads remain in GLB BIN', assets: reports }, null, 2) + '\n');
