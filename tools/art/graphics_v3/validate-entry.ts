...
const limits: Record<string,number> = {
  player:60000,raider:60000,scavenger:50000,'scrap-rifle':12000,'scrap-shotgun':12000,
  'salvaged-radio':12000,'manual-turret':26000,'expedition-wreck':105000,'raider-skiff':75000,
  'machine-walker-v3':120000,'machine-kit':32000,'station-kit':16000,'salvage-chest':15000,'forged-hook':4000,
};
...
async function main() {
  ...
  for(const [id,limit] of Object.entries(limits)) {
    ...
    if(id==='manual-turret'){
      for(const name of ['TurretYaw','TurretPitch','Muzzle'])check(root.getObjectByName(name),`${id}: ${name}`);
      const yaw=root.getObjectByName('TurretYaw'),muzzle=root.getObjectByName('Muzzle');
      if(yaw&&muzzle){
        const before=muzzle.getWorldPosition(new THREE.Vector3()),angle=yaw.rotation.y;
        yaw.rotation.y+=Math.PI/3;root.updateMatrixWorld(true);
        check(before.distanceTo(muzzle.getWorldPosition(new THREE.Vector3()))>.1,'turret: live muzzle follows yaw');
        yaw.rotation.y=angle;root.updateMatrixWorld(true);
      }
    }
    ...
    if(['player','raider','scavenger'].includes(id)){
      if(id!=='player')for(const name of ['Climb','Punch','Death'])check(model.clips.some(c=>c.name===name),`${id}: ${name} clip`);
      ...
      const walking=model.clips.find(c=>c.name==='Walking');
      if(walking){
        const actor=clone(root),mixer=new THREE.AnimationMixer(actor);
        const joint=actor.getObjectByName('ThighL')??actor.getObjectByName('Thigh.L');
        const original=joint?root.getObjectByName(joint.name):null;
        if(joint&&original){
          const rest=joint.quaternion.clone();mixer.clipAction(walking).play();mixer.setTime(walking.duration/4);
          check(rest.angleTo(joint.quaternion)>.1,`${id}: walking changes thigh pose`);
          check(original.quaternion.angleTo(joint.quaternion)>.1,`${id}: cloned skeleton is independent`);
        }else check(false,`${id}: thigh joint resolves`);
        mixer.stopAllAction();
      }
