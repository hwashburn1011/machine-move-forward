...
try {
  ...
  await page.route('**/models/authored/*.glb', async (route) => {
    ...
    const path = `assets/graphics-v3/optimized/${file}`;
  ...
  await page.route('**/art-fixture', (route) => route.fulfill({
    ...
    body: '<!doctype html><html><body><script type="module" src="/tools/art/graphics_v3/poses-entry.ts"></script></body></html>',
  }));
  ...
  await page.screenshot({ path: 'docs/art/graphics-v3/player-poses-front.png' });
  await page.evaluate(() => {
  ...
  await page.screenshot({ path: 'docs/art/graphics-v3/player-poses-back.png' });
