...
try{
 ...
 check('new skinned character is live',initial.skinnedTriangles>=52000,initial.skinnedTriangles);
 ...
 await page.waitForTimeout(200);await page.screenshot({path:'docs/art/graphics-v3/after-bypass.png'});
 await page.evaluate(()=>{const g=globalThis.__game.game;g.post.setBypassed(false);g.queueDebugAction('time');});
 await page.waitForTimeout(600);await page.screenshot({path:'docs/art/graphics-v3/after-afternoon.png'});
 await page.evaluate(()=>{
 ...
 await page.screenshot({path:'docs/art/graphics-v3/after-combat.png'});
 check('no browser errors',errors.length===0,errors);
 await writeFile('docs/art/graphics-v3/runtime-acceptance.json',JSON.stringify({generatedAt:new Date().toISOString(),initial,quality,combat,checks,errors,method:'Real hardware Chrome 1080p High; explicit staged full eight-enemy pool plus boarding skiff stress, not a naturally played encounter.'},null,2)+'\n');
