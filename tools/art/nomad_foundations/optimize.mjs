...
const names = ['salvage-chest','fuel-canister','generator-refined','salvaged-radio','wrist-terminal','workbench-refined','refinery-refined','storage-refined'];
const report = [];
async function writeRetry(path,data){
  for(let attempt=0;attempt<40;attempt++){
    try { await writeFile(path,data); return; }
    catch(error) { if(attempt===39)throw error; await new Promise(resolve=>setTimeout(resolve,150)); }
  }
}
const pad4 = (b) => Buffer.concat([b, Buffer.alloc((4 - (b.length % 4)) % 4)]);
for (const name of names) {
  const source = 'assets/nomad-foundations/exports/' + name + '.glb';
  try {
  ...
  await writeRetry('public/models/authored/' + name + '.glb', out);
  report.push({
    name,
...
await writeRetry('assets/nomad-foundations/optimization.json', JSON.stringify(report, null, 2));
