...
try{
 ...
 if(kind)await page.route('**/models/authored/*.glb',async route=>{
  const name=new URL(route.request().url()).pathname.split('/').at(-1),path=`assets/graphics-v2/${kind}/${name}`;
 ...
 await page.route('**/validate-fixture',route=>route.fulfill({contentType:'text/html',body:'<!doctype html><html><body><script type="module" src="/tools/art/graphics_v2/validate-entry.ts"></script></body></html>'}));
 ...
 result.errors.push(...pageErrors);await mkdir('docs/art/graphics-v2',{recursive:true});
 await writeFile(`docs/art/graphics-v2/asset-validation-${kind??'runtime'}.json`,JSON.stringify(result,null,2)+'\n');
 for(const id of ['salvaged-radio','manual-turret','expedition-wreck','raider-skiff','station-kit','salvage-chest']){
  ...
  await page.screenshot({path:`docs/art/graphics-v2/${id}-${kind??'runtime'}.png`});
 }
