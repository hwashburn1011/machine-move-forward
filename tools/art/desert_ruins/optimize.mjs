/**
 * Optimize the desert ruin library without changing its authored graph.
 *
 * The pass keeps original texture masters untouched, adds high-quality
 * WebP images as required texture sources, and encodes geometry bufferViews with
 * EXT_meshopt_compression. Both Blender-authored detail levels keep their
 * geometry, pivots, normals and UVs through this compression pass.
 *
...
const INPUT = path.join(ROOT, 'assets', 'desert-ruins', 'exports');
...
function makeGlb(json, bin) {
  ...
  const jsonBytes = Buffer.from(jsonText + ' '.repeat((4 - (jsonText.length % 4)) % 4));
    ...
    for (const primitive of mesh.primitives || []) {
      for (const accessorIndex of [
        ...
        if (prior && prior.accessorIndex !== accessorIndex) {
          ...
          byView.set(viewIndex, {
            accessorIndex,
            mode: accessorIndex === primitive.indices ? 'TRIANGLES' : 'ATTRIBUTES',
          });
        }
      }
    }
...
async function optimizeOne(file) {
    ...
    if (info && !info.unsupported && !view.byteStride) {
      ...
      outView.extensions = {
        EXT_meshopt_compression: {
          buffer: 0,
          byteOffset: compressedChunk.offset,
  ...
  for (let imageIndex = 0; imageIndex < originalImages.length; imageIndex++) {
    ...
    if (
      ...
      throw new Error(`Unsupported embedded image: ${image.name}`);
    ...
    const prepared = smallDetail
      ? sharp(png).resize({ width: 1024, height: 1024, fit: 'fill' })
      : sharp(png);
    const webp = await prepared
      .webp(isBase ? { quality: 92, effort: 5 } : { lossless: true, effort: 5 })
      .toBuffer();
    const chunk = addChunk(parts, webp);
    const webpIndex = webpEntries.length;
    webpEntries.push({
      name: `${image.name || `image_${imageIndex}`}_webp`,
      bufferView: newViews.length,
      mimeType: 'image/webp',
    });
    ...
    imageReport.push({
      ...
      roles: role,
      width: smallDetail ? 1024 : meta.width,
      height: smallDetail ? 1024 : meta.height,
  ...
  json.buffers.push({
    byteLength: fallbackCursor,
    extensions: { EXT_meshopt_compression: { fallback: true } },
  });
  json.bufferViews = newViews;
  json.buffers[0].byteLength = parts.bytes;

  const rootNode = (json.nodes || []).find(
    (node) => /^MMF_[^_]+$/.test(node.name || '') || node.extras?.graphicsVersion,
  );
  const rootExtras = rootNode?.extras || {};
  const stem = path.basename(file, '.glb');
  const archetypes = JSON.parse(
    await fs.readFile(path.join(ROOT, 'assets/desert-ruins/source/model-report.json'), 'utf8'),
  );
  const required = archetypes.flatMap((model) => [model.name, model.name + '__lod']);
  ...
  return {
    ...
    webpTextureBytes: textureWebPBytes,
    selectedTextureGpuBytesWithMips: Math.round(gpu),
    images: imageReport,
    requiredNodes: required,
    extras: rootExtras,
    lod: {
      source: 'Blender Decimate',
      ratio: 0.32,
      sharedOrigins: true,
      modelCount: archetypes.length,
    },
    output: outPath,
  };
}
...
const selected = args.length
  ? args.map((name) => path.resolve(INPUT, name.endsWith('.glb') ? name : `${name}.glb`))
...
await fs.writeFile(
  path.join(ROOT, 'assets/desert-ruins/source/optimize-manifest.json'),
  JSON.stringify(
    {
      version: 1,
      generatedAt: new Date().toISOString(),
      encoder: 'meshoptimizer 1.1.1',
      texturePolicy:
        'optimized GLBs require EXT_texture_webp and select baseColor WebP q92 at 2K and normal/ORM lossless WebP at 1K; original 2K texture masters remain in assets/desert-ruins/textures; WebP does not reduce GPU residency',
      geometryPolicy:
        'EXT_meshopt parents use a placeholder fallback buffer sized for decoded accessors; compressed payloads remain in GLB BIN',
      assets: reports,
    },
    null,
    2,
