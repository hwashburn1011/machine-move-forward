...
with bpy.data.libraries.load(str(root/'assets/glass-orchard/glass-orchard.blend'),link=False) as (source,target):
    target.scenes=source.scenes[:1]
scene=target.scenes[0];scene.name='Glass Orchard campaign review'
camera_data=bpy.data.cameras.new('Orchard review camera')
camera=bpy.data.objects.new('Orchard review camera',camera_data);scene.collection.objects.link(camera)
