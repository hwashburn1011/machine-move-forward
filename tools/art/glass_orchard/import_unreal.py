...
ROOT=Path(__file__).resolve().parents[3];OUT=ROOT/'assets/glass-orchard'
...
try:
    for file in sorted((OUT/'exports').glob('*.fbx')):
        ...
        task=unreal.AssetImportTask();task.filename=str(file);task.destination_path='/Game/GlassOrchard'
        ...
        for path in task.imported_object_paths:
            ...
            assert 100 < max(size) < 5000, ('Expected metre-authored geometry in centimetres',size)
            report['assets'].append({'name':file.stem,'path':path,'size_cm':size,'materials':len(asset.get_editor_property('static_materials'))})
    assert len(report['assets'])==3, report
    editor=unreal.get_editor_subsystem(unreal.LevelEditorSubsystem)
    editor.new_level('/Game/GlassOrchard/Review')
    actors=unreal.get_editor_subsystem(unreal.EditorActorSubsystem)
    for i,entry in enumerate(report['assets']):
        actor=actors.spawn_actor_from_class(unreal.StaticMeshActor,unreal.Vector(i*3000,0,1500))
...
unreal.log('GLASS_ORCHARD_IMPORT '+json.dumps(report))
