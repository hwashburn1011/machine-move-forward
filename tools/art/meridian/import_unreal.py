...
ROOT=Path(__file__).resolve().parents[3];OUT=ROOT/'assets/meridian'
...
try:
    for file in sorted((OUT/'exports').glob('*.fbx')):
        ...
        task=unreal.AssetImportTask();task.filename=str(file);task.destination_path='/Game/Meridian'
        ...
        for path in task.imported_object_paths:
            ...
            assert 100 < max(size) < 12000, ('Expected metre-authored geometry in centimetres',size)
    ...
    editor.new_level('/Game/Meridian/Review')
...
unreal.log('MERIDIAN_IMPORT '+json.dumps(report))
