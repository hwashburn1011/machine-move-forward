...
const INPUT = process.env.MMF_GLTF_INPUT || path.join(ROOT, 'assets', 'mech-enemies', 'exports');
const OUTPUT = process.env.MMF_GLTF_OUTPUT || path.join(ROOT, 'assets', 'mech-enemies', 'optimized');
...
await fs.writeFile(process.env.MMF_GLTF_REPORT || path.join(OUTPUT, 'optimize-manifest.json'), JSON.stringify({ version: 1, generatedAt: new Date().toISOString(), encoder: 'meshoptimizer 1.1.1', texturePolicy: 'optimized GLBs require EXT_texture_webp and select baseColor/emissive WebP q92 at 1K or normal/ORM lossless WebP at 512; original 1K PNG masters remain in staging; WebP does not reduce GPU residency', geometryPolicy: 'EXT_meshopt parents use a placeholder fallback buffer sized for decoded accessors; compressed payloads remain in GLB BIN', assets: reports }, null, 2) + '\n');
