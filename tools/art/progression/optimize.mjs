...
const names = ['nomad-progress'];
const report = [];
const pad4 = (b) => Buffer.concat([b, Buffer.alloc((4 - (b.length % 4)) % 4)]);
for (const name of names) {
  const source = 'assets/progression/exports/' + name + '.glb';
  try {
...
await writeFile('assets/progression/optimization.json', JSON.stringify(report, null, 2));
